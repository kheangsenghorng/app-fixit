...
import '../../../../home/data/provider/service_local_storage_provider.dart';
import '../repository/service_repository_provider.dart';
...
class ServiceNotifier extends AsyncNotifier<List<Service>> {
  ...
  Future<List<Service>> _fetchServicesByType(int typeId) async {
    final repository = ref.read(serviceRepositoryProvider);
    return repository.getServiceByType(typeId);
  }
  ...
  Future<List<Service>> _refreshFromApi({required int typeId}) async {
    final services = await _fetchServicesByType(typeId);
    await _saveLocalServices(services);
    return services;
  }

  Future<void> _refreshFromApiSilently() async {
    try {
      if (_currentTypeId == null) return;

      final services = await _fetchServicesByType(_currentTypeId!);
      await _saveLocalServices(services);
  ...
  bool _matchesCurrentType(Service service) {
    return _currentTypeId != null &&
        service.category.id == _currentTypeId;
  }
  ...
  void _syncCreatedService(Service created) {
    ...
    if (!_matchesCurrentType(created)) return;
  ...
  void _syncUpdatedService(Service updated) {
    ...
    final matchesType = _matchesCurrentType(updated);

    List<Service> next;

    if (isActive && matchesType) {
      next = exists
  ...
  Future<void> fetchByType(int typeId) async {
    _currentTypeId = typeId;
    state = const AsyncLoading();
    state = await AsyncValue.guard(
          () => _refreshFromApi(typeId: typeId),
    );
  }
  ...
  Future<void> refreshServices() async {
    ...
    state = const AsyncLoading();
    state = await AsyncValue.guard(
          () => _refreshFromApi(typeId: _currentTypeId!),
    );
  }
}