import 'package:fixit/features/user/profile/widgets/profile_menu_tile.dart';
import 'package:flutter/material.dart';
import 'package:flutter_riverpod/flutter_riverpod.dart';

import '../../../../core/models/user_model.dart';
import '../../../../core/provider/language_provider.dart';
import '../../../../core/provider/theme_provider.dart';
import '../../../../l10n/app_localizations.dart';
import '../../../../routes/app_routes.dart';
import '../../../auth/presentation/providers/auth_controller.dart';
import 'language_picker_sheet.dart';
import 'logout_dialog.dart';

class ProfileMenuSection extends ConsumerWidget {
  final UserModel user;
  final Color contentColor; // Used for monochrome text

  const ProfileMenuSection({
    super.key, 
    required this.user, 
    required this.contentColor, 
  });

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final themeMode = ref.watch(themeNotifierProvider);
    final currentLocale = ref.watch(languageNotifierProvider);
    final l10n = AppLocalizations.of(context);

    String getModeName(ThemeMode mode) {
      if (mode == ThemeMode.light) return l10n.t('light_mode');
      if (mode == ThemeMode.dark) return l10n.t('dark_mode');
      return l10n.t('system_default');
    }

    return Column(
      children: [
        const SizedBox(height: 25),

        ProfileMenuTile(
          icon: Icons.person_outline,
          iconColor: Colors.redAccent, // COLOR KEPT
          title: l10n.t('edit_profile'),
          subtitle: user.name,
          onTap: () => Navigator.pushNamed(context, AppRoutes.editProfile, arguments: user),
        ),

        ProfileMenuTile(
          icon: Icons.notifications_none,
          iconColor: Colors.blueAccent, // COLOR KEPT
          title: l10n.t('notification'),
          onTap: () {},
        ),

        ProfileMenuTile(
          icon: Icons.palette_outlined,
          iconColor: Colors.amber, // COLOR KEPT
          title: l10n.t('appearance'),
          subtitle: getModeName(themeMode),
          onTap: () => Navigator.pushNamed(context, AppRoutes.appearance),
        ),

        ProfileMenuTile(
          icon: Icons.language,
          iconColor: Colors.blue, // COLOR KEPT
          title: l10n.t('language'),
          subtitle: currentLocale.languageCode == 'km' ? 'ភាសាខ្មែរ' : 'English',
          onTap: () => _showLanguagePicker(context),
        ),

        ProfileMenuTile(
          icon: Icons.headset_mic_outlined,
          iconColor: Colors.blueGrey, // COLOR KEPT
          title: l10n.t('help_support'),
          onTap: () => Navigator.pushNamed(context, AppRoutes.helpSupport),
        ),

        const Padding(
          padding: EdgeInsets.symmetric(horizontal: 20),
          child: Divider(height: 30, thickness: 0.5),
        ),

        ProfileMenuTile(
          icon: Icons.logout,
          iconColor: Colors.red, // COLOR KEPT
          title: l10n.t('logout'),
          showArrow: false,
          onTap: () => _handleLogout(context),
        ),

        const SizedBox(height: 25),
      ],
    );
  }

  void _showLanguagePicker(BuildContext context) {
    showModalBottomSheet(
      context: context,
      backgroundColor: Colors.transparent,
      builder: (_) => const LanguagePickerSheet(),
    );
  }

  void _handleLogout(BuildContext context) {
    showDialog(
      context: context,
      builder: (dialogContext) => LogoutDialog(
        onConfirm: () async {
          Navigator.pop(dialogContext);
          showDialog(
            context: context,
            barrierDismissible: false,
            builder: (context) => Center(
              child: CircularProgressIndicator(color: contentColor),
            ),
          );
          
          await Future.delayed(const Duration(seconds: 2));

          if (context.mounted) {
            final ref = ProviderScope.containerOf(context);
            await ref.read(authControllerProvider.notifier).logout();
            if (context.mounted) Navigator.of(context).pop();
          }
        },
      ),
    );
  }
}